ssetwd("~/Documents/manuscripts/XQTL/G3_submission/github/final/XQTL-mapping/data-files")
library(tidyverse)

#full list of 2701 alternate alleles
...
# quantify the number of variants with each type of effect
select(merged,ID, Consequence) %>% distinct() %>% pull(Consequence) %>% table()
...
#how many of each class of variant
pull(variant.tbl,variant_type) %>% table() %>% sort(decreasing=TRUE)
#percentage of variants that are SNps and small INDELs
(1211+772)/2050

pull(variant.tbl,functional) %>% table() %>% sort(decreasing=TRUE)
#percentage of variants that are SNps and small INDELs
1215/2050
