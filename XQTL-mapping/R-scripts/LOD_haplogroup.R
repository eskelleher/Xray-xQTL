...
##lod peak in 500 Kb mapping is chr3R 3317440
peak3R.500.Fs <- filter(Fs.500, chr == "chr3R", pos == 3317440)
#change B2 to B6 because these two haplotygroups cannot be separated and this more logical colors in the combined plot
peak3R.500.Fs$haplogroup[peak3R.500.Fs$haplogroup=="B2"] <- "B6"
...
#change B2 to B6 because these two haplotygroups cannot be separated and this more logical colors in a combined plot
peak3R.500.Fs$haplogroup[peak3R.500.Fs$haplogroup=="B2"] <- "B6"


##lod peak in 500 Kb mapping is chr3L 3317440
peak3L.500.Fs <- filter(Fs.500, chr == "chr3L", pos == 25716280)

###inspect the raw haplotype frequencies at this position to determine who should be dropped and which haplotypes are grouped
filter(hap500, chr == "chr3L", pos == 25716280)
#B1/B5 are basically absent freq < 0.02
#all other haplotypes can be resolved

#make a combined table for a faceted plot 
peak.500.Fs <- rbind(peak3R.500.Fs,peak3L.500.Fs)
...
ggplot(filter(peak.500.Fs,haplogroup != "B1", haplogroup != "B5"), aes(haplogroup, freqdff, colour = haplogroup)) +
  ...
  geom_point(size=1.5) +
  scale_color_manual(values=c("#db715b","#86443c","#332f2c","#338a7d","#Db2f18","#866e3c")) +theme(legend.position = "none") +
  geom_hline(yintercept=1,linetype=2) +
  facet_grid(~chr,scales="free",space="free") + 
  xlab("\nfounder haplotype") + ylab("relative haplotype frequency (T/C)\n") 
ggsave("deltafreq.pdf", height =3, width=3.5)
...
peak.3R.500 <- filter(cent.500, chr == "chr3R", pos == 3317440,haplogroup != "B1",haplogroup != "B5") %>% separate(pool,c("rep","trt")) %>% filter(trt=="C")
peak.3R.500$haplogroup[peak.3R.500$haplogroup=="B2"] <- "B6"
peak.3L.500 <- filter(cent.500, chr == "chr3L", pos == 25716280,haplogroup != "B1",haplogroup != "B5") %>% separate(pool,c("rep","trt")) %>% filter(trt=="C")
peak.500 <- rbind(peak.3R.500,peak.3L.500)
ggplot(peak.500, aes(x=rep,y=freq_sum,fill=haplogroup)) + 
  geom_bar(stat="identity") + 
  scale_fill_manual(values=c("#db715b","#86443c","#332f2c","#338a7d","#Db2f18","#866e3c")) +
  theme_classic() + xlab("\nreplicate") +
  facet_grid(~chr)+
  ylab("haplogroup frequency\n")

ggsave("haplogroup_freq.pdf", height = 3, width = 4 )
